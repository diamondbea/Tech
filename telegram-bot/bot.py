...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "2. Send: /setkey r8_yourkey\n"
        "3. Send: /imagine a glowing cyberpunk city at night\n\n"
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "Edit a photo (send photo first, then command):\n"
        "/animate - animate it\n"
        "/upscale - upscale 4x\n"
        "/removebg - remove background\n"
        "/restore - restore and enhance faces\n"
...
async def cmd_setkey(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ctx.args:
        await update.message.reply_text("Usage: /setkey r8_yourreplicatekey")
        return
    ...
    if not key.startswith("r8_"):
        ...
        return
    user_keys[uid] = key
    await update.message.reply_text("Key saved! Try /imagine a sunrise over the mountains")
...
async def cmd_imagine(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not key:
        await update.message.reply_text(
            "First save your Replicate key:\n/setkey r8_yourkey\n\nGet one free at replicate.com/account/api-tokens"
        )
        return
    prompt = " ".join(ctx.args).strip()
    if not prompt:
        await update.message.reply_text("Usage: /imagine a hyper-realistic warrior portrait")
        return
    ...
    try:
        pred = await replicate_create(
            ...
            {"prompt": prompt, "aspect_ratio": aspect, "num_inference_steps": 4,
...
async def handle_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    pending_photo[update.effective_user.id] = update.message.photo[-1].file_id
    await update.message.reply_text(
        "Photo received! Now send:\n"
        "/animate - bring it to life\n"
        "/upscale - sharpen and enlarge 4x\n"
        "/removebg - remove background\n"
        "/restore - restore and enhance faces\n"
        "/repaint <instruction> - repaint with AI"
    )
...
async def cmd_animate(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    key = user_keys.get(uid)
    if not key:
        await update.message.reply_text("First save your key: /setkey r8_yourkey")
        return
    ...
    try:
        ...
        pred = await replicate_create(
            ...
            {"input_image": img, "motion_bucket_id": 127, "fps_id": 12,
             "num_frames": 25, "decoding_t": 7, "output_format": "mp4"},
            key,
        )
        url = first_output(pred)
        if not url:
...
async def cmd_restore(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not file_id:
        ...
        return
    msg = await update.message.reply_text("Restoring and enhancing...")
    await update.effective_chat.send_action(ChatAction.UPLOAD_PHOTO)
    try:
        img = await tg_photo_to_b64(file_id, ctx.bot)
        pred = await replicate_create(
